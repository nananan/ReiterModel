...
class Slave: public AbstractProcess{
    private:
        ...
        MPI_Request request[4] = {0}, send_req[4];
    ...
    public:
        ...
        void sendReceiveBorderFirst(MPI_Status status, bool firstTime) {
            int num_col = block.column;
            int num_row = block.row;
            int init = 0;
            if (!firstTime) {
        ...
        void sendReceiveBorder(MPI_Status status, bool firstTime) {
            ...
            if (nbrs[UP] != -2) {
                ...
                MPI_Isend(&(tmp[0]), 1, rowtype, neighbour[0], BORDER, cartcomm, &send_req[0]);
                MPI_Irecv(bord_horizontal, num_col, MPI_FLOAT, neighbour[0], BORDER, cartcomm, &request[0]);
                MPI_Isend(&(tmp_U[0]), 1, rowtype, neighbour[0], BORDER_U, cartcomm, &send_req[0]);
                MPI_Irecv(bord_horizontal_U, num_col, MPI_FLOAT, neighbour[0], BORDER_U, cartcomm, &request[0]);
                delete [] tmp;
                delete [] tmp_U;
            }
            if (nbrs[DOWN] != -2) {
                ...
                MPI_Irecv(bord_horizontal, num_col, MPI_FLOAT, neighbour[0], BORDER, cartcomm, &send_req[1]);
                MPI_Isend(&(tmp[0]), 1, rowtype, neighbour[0], BORDER, cartcomm, &request[1]);
                MPI_Irecv(bord_horizontal_U, num_col, MPI_FLOAT, neighbour[0], BORDER_U, cartcomm, &send_req[1]);
                MPI_Isend(&(tmp_U[0]), 1, rowtype, neighbour[0], BORDER_U, cartcomm, &request[1]);
                delete [] tmp;
                delete [] tmp_U;
            }
            if (nbrs[LEFT] != -2) {
                MPI_Isend(&data[0*num_col+0], 1, columntypeBorder, neighbour[1], BORDER, cartcomm, &send_req[2]);
                MPI_Irecv(bord_vertical, num_row, MPI_FLOAT, neighbour[1], BORDER, cartcomm, &request[2]);
                MPI_Isend(&data_U[0*num_col+0], 1, columntypeBorder, neighbour[1], BORDER_U, cartcomm, &send_req[2]);
                MPI_Irecv(bord_vertical_U, num_row, MPI_FLOAT, neighbour[1], BORDER_U, cartcomm, &request[2]);
            }
            if (nbrs[RIGHT] != -2) {
                MPI_Irecv(bord_vertical, num_row, MPI_FLOAT, neighbour[1], BORDER, cartcomm, &send_req[3]);
                MPI_Isend(&data[0*num_col+(num_row-1)], 1, columntypeBorder, neighbour[1], BORDER, cartcomm, &request[3]);
                MPI_Irecv(bord_vertical_U, num_row, MPI_FLOAT, neighbour[1], BORDER_U, cartcomm, &send_req[3]);
                MPI_Isend(&data_U[0*num_col+(num_row-1)], 1, columntypeBorder, neighbour[1], BORDER_U, cartcomm, &request[3]);
            }
        }
        ...
        void update(int step) {
            ...
            grid_new = new float*[row+1];
            U_new = new float*[row+1];
            V_new = new float*[row+1];
            for (int i = 0; i < row+1; i++) {
                grid_new[i] = new float[column+1];
                U_new[i] = new float[column+1];
                V_new[i] = new float[column+1];
            }
            int index_row = 0;
            int index_column = 0;
            int end_row = 0;
            ...
            for (int i = index_row; i < end_row; i++) {
                for (int j = index_column; j < end_column; j++) {
                    bool recep = false;
                    if (grid[i][j] >= 1)
                        recep = true;
                    else {
        ...
        void updateNeighbour(bool horizontal) {
            int row = block.row;
            int column = block.column;
